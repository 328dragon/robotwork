...
int main(void)
{
  ...
  MX_TIM15_Init();
  /* USER CODE BEGIN 2 */
    //tim1是给两个普通直流电机的，tim3是编码器1，tim5是编码器2，tim2只用于普通定时（利用中断）,tim15是为了调频率用的
    //tim6是给ccd定时中断用，tim8是ccd的m脚,tim7是为了让sh方便取样所用定时器,tim4是用于输出pwm  配置：340-1，10000-1
//  中断开启
